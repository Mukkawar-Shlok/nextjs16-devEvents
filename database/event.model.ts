 ...
 * Includes automatic slug generation, date normalization, and validation
 */
 ...
 * - Ensures time is in consistent 24-hour format
 */
EventSchema.pre("save", function (next) {
  ...
  // Normalize time to 24-hour HH:MM format if modified
  if (this.isModified("time")) {
    ...
    if (!timeRegex.test(this.time)) {
      ...
      try {
        ...
        return next(
          new Error("Time must be in HH:MM format (24-hour) or HH:MM AM/PM")
        );
      }
    }
  }
...
// Create unique index on slug for efficient lookups and uniqueness enforcement
